...
const iconUrl = (href) => {
	...
	return `https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${origin}&size=16`
}
...
const renderNode = (root) => ({ title, children, url: href }) => {
	...
	if (children) {
		...
		const icon = `<img src="${iconUrl(href)}" alt="${title.substring(0, 1)}" />`
